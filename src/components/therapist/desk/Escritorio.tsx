...
function CarpetaCard({ p, isDemo, onOpen }: { p: Patient; isDemo: boolean; onOpen: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        ...
        {isDemo && <span style={{ marginLeft: 'auto' }}><EjemploTag /></span>}
      </div>
    </button>
  )
}
    ...
    <div style={{ width: '100%', maxWidth: '1000px', margin: '0 auto', padding: '28px 20px 48px', fontFamily: DT.body }}>
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '14px' }}>
          {filtered.map(p => <CarpetaCard key={p.id} p={p} isDemo={isDemo} onOpen={() => onOpen(p.id)} />)}
        </div>
      )}
    </div>
  )
