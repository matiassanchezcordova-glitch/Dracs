...
import { Gamepad2, Heart, Stethoscope } from 'lucide-react'
...
// ── Login link ────────────────────────────────────────────────────────────────
// Discreto a propósito: el camino principal es la demo, sin cuenta. El login
// sigue vivo para pilotos reales.
...
// ── Role card ─────────────────────────────────────────────────────────────────
// Las tres puertas del showroom. El niño ve "Juegos" (nunca "Ejercicios").

export default function RoleCard({ onSelect, onLogin }: { onSelect: (role: Role) => void; onLogin: () => void }) {
  return (
    <div className="dracs-role-card" style={{ maxWidth: '400px', width: '100%', background: '#ffffff', borderRadius: '24px', boxShadow: '0 20px 60px rgba(0,0,0,0.08)', padding: '28px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
      ...
      <RoleRowBtn icon={<Heart size={18} />}       iconBg="#F0FDF4" iconBorder="#BBF7D0" iconColor="#059669" title="Familia"   subtitle="Cómo va en casa"      onClick={() => onSelect('family')} />
      <RoleRowBtn icon={<Stethoscope size={18} />} iconBg="#EAF3F5" iconBorder="#EAF3F5" iconColor="#5B8896" title="Logopeda"  subtitle="Escritorio clínico"   onClick={() => onSelect('therapist')} />
